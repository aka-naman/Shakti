/**
 * PIS Bridge - Integration between Noting_builder and Agra-sandhani
...
const PISBridge = {
    ...
    init: function(options) {
        ...
        if (!this.config.agraImportColumnsFormId) {
            this.config.agraImportColumnsFormId = this.config.agraImportFormId || this.config.agraFormId;
        }
        if (!this.config.agraImportVariablesFormId) {
            this.config.agraImportVariablesFormId = this.config.agraImportFormId || this.config.agraFormId;
        }
        if (!this.config.agraExportColumnsFormId) {
            this.config.agraExportColumnsFormId = this.config.agraExportFormId || this.config.agraFormId;
        }
        if (!this.config.agraExportVariablesFormId) {
            this.config.agraExportVariablesFormId = this.config.agraExportFormId || this.config.agraFormId;
        }
     ...
     */
    fetchData: async function(pisValue, bypassCache = false, isColumn = false) {
        if (!pisValue || pisValue.trim() === '') return null;
        
        const formId = isColumn ? 
            (this.config.agraImportColumnsFormId || this.config.agraImportFormId || this.config.agraFormId) : 
            (this.config.agraImportVariablesFormId || this.config.agraImportFormId || this.config.agraFormId);
        ...
        console.log('[PIS Bridge] Fetching from API:', pisValue, 'for Form ID:', formId);
    ...
    searchPIS: async function(query, isColumn = false) {
        ...
        console.log('[PIS Bridge] Searching for:', query, 'on Form ID:', formId);
    ...
    fillPlainForm: function(fetchedData, triggerFieldKey = null) {
        ...
        inputs.forEach(input => {
            ...
            // Skip nominee table inputs
            if (input.classList.contains('nominee-cell-input')) return;
    ...
    attachToInput: function(input, isPlainForm = false) {
        ...
        const isColumn = input.classList.contains('nominee-cell-input');
        ...
        const triggerSearch = async () => {
            ...
            const results = await self.searchPIS(query, isColumn);
            ...
            if (results.length > 0) {
                ...
                results.forEach((r, idx) => {
                    items[idx].onclick = async (ev) => {
                        ...
                        // 2. Fetch full record
                        const fullResult = await self.fetchData(r.pis, false, isColumn);
                        if (!fullResult) {
                            input.value = r.pis;
                            return;
                        }
                        ...
                        if (isPlainForm) {
                            self.fillPlainForm(fullResult, triggerKey);
                        } else {
                            self.fillRow(rowIndex, fullResult, colName);
                            if (!isColumn) {
                                self.fillPlainForm(fullResult);
                            }
                        }
        ...
        input.addEventListener('blur', async () => {
            ...
            setTimeout(async () => {
                if (list.style.display === 'none' && input.value.length >= 2) {
                    const result = await self.fetchData(input.value, false, isColumn);
                    if (result) {
                        const colName = input.getAttribute('data-col') || input.id || input.name;
                        const triggerKey = self.getMapKeyForInput(input);
                        ...
                        if (isPlainForm) {
                            self.fillPlainForm(result, triggerKey);
                        } else {
                            self.fillRow(rowIndex, result, colName);
                            if (!isColumn) {
                                self.fillPlainForm(result);
                            }
                        }
                    }
                }
        ...
        if (parent && !parent.querySelector('.btn-pis-fetch')) {
            ...
            btn.onclick = async (e) => {
                e.stopPropagation();
                const result = await self.fetchData(input.value, true, isColumn);
                if (result) {
                    const colName = input.getAttribute('data-col') || input.id || input.name;
                    const triggerKey = self.getMapKeyForInput(input);
                    if (isPlainForm) {
            ...
            // Absolute position the fetch button inside the input wrapper to prevent overlapping
    ...
    /**
     * Helper to submit prefill session and open in new tab
     */
    submitPrefillAndRedirect: async function(formId, mappedPrefill) {
        try {
            ...
            // Redirect browser to Agra-sandhani Form submit page with prefillToken
            const port = 5000;
            const agraHost = `${window.location.protocol}//${window.location.hostname}:${port}`;
            const targetUrl = `${agraHost}/forms/${formId}/submit?prefillToken=${token}`;

            console.log('[PIS Bridge] Redirecting to:', targetUrl);
            window.open(targetUrl, '_blank');
        } catch (e) {
            alert(`Error exporting to form ${formId}: ${e.message}`);
        }
    },

    /**
     * Exports noting data to Agra-sandhani by generating prefill token sessions
     * and redirecting the browser to the prefilled forms.
     * @param {Object} rowData Nominee row details (optional)
     */
    exportToForm: async function(rowData = null) {
        const exportColumnsFormId = this.config.agraExportColumnsFormId || this.config.agraExportFormId || this.config.agraFormId;
        const exportVariablesFormId = this.config.agraExportVariablesFormId || this.config.agraExportFormId || this.config.agraFormId;

        if (!exportColumnsFormId && !exportVariablesFormId) {
        ...
        // Helper to map a subset of data using exportMap
        const getMappedData = (sourceData) => {
            const mapped = {};
            Object.entries(sourceData).forEach(([notingVar, val]) => {
                if (!val) return;
        ...
        if (exportColumnsFormId === exportVariablesFormId) {
            ...
            const mappedPrefill = getMappedData(mergedData);
            ...
            await this.submitPrefillAndRedirect(exportColumnsFormId, mappedPrefill);
        } else {
            let columnPrefill = {};
            let variablePrefill = {};

            if (rowData && exportColumnsFormId) {
                columnPrefill = getMappedData(rowData);
            }
            if (exportVariablesFormId) {
                variablePrefill = getMappedData(flatValues);
            }

            if (Object.keys(columnPrefill).length === 0 && Object.keys(variablePrefill).length === 0) {
                alert("None of the fields on this form are currently mapped to either target Agra-sandhani form. Please configure mapping in Master Manager first.");
                return;
            }

            // Export Columns if mapped
            if (exportColumnsFormId && Object.keys(columnPrefill).length > 0) {
                console.log('[PIS Bridge] Exporting Columns to Form ID:', exportColumnsFormId, columnPrefill);
                await this.submitPrefillAndRedirect(exportColumnsFormId, columnPrefill);
            }

            // Export Variables if mapped (wait slightly to avoid browser popup blocker blocking one of the tabs)
            if (exportVariablesFormId && Object.keys(variablePrefill).length > 0) {
                console.log('[PIS Bridge] Exporting Variables to Form ID:', exportVariablesFormId, variablePrefill);
                setTimeout(async () => {
                    await this.submitPrefillAndRedirect(exportVariablesFormId, variablePrefill);
                }, 400);
            }
        }
    },
    ...
    /**
     * Exports only noting document variables to the Variables Form.
     */
    exportVariablesOnly: async function() {
        const exportVariablesFormId = this.config.agraExportVariablesFormId || this.config.agraExportFormId || this.config.agraFormId;

        if (!exportVariablesFormId) {
            alert("No target Agra-sandhani form mapped for variables.");
            return;
        }
        ...
        // Map Noting variables to Agra-sandhani labels using exportMap
        const variablePrefill = {};
        Object.entries(flatValues).forEach(([notingVar, val]) => {
            if (!val) return;
            const targetMapKey = Object.keys(explicitMap).find(k => normalize(k) === normalize(notingVar));
            if (targetMapKey) {
                const agraLabel = explicitMap[targetMapKey];
                variablePrefill[agraLabel] = val;
            }
        });

        console.log('[PIS Bridge] Exporting variables only:', variablePrefill);

        if (Object.keys(variablePrefill).length === 0) {
            alert("None of the document variables on this form are currently mapped to the target Agra-sandhani form. Please configure mapping in Master Manager first.");
            return;
        }
